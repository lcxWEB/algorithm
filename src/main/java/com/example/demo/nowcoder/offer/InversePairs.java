...
/**
 * Created by lichunxia on 2020-01-28 18:29
 */
public class InversePairs {

    private int count = 0;
    private int modBy = 1000000007;
     ...
     * @return
     */
    public int InversePairs213(int[] array) {
        if (array == null || array.length <= 1) {
            return 0;
        }
        int[] merge = new int[array.length];
        int result = InversePairs(array, 0, array.length - 1, merge);
        return result % modBy;
    }

    private int InversePairs(int[] array, int left, int right, int[] merge) {
        if (left >= right) {
            return 0;
        }
        ...
        int leftPairs = InversePairs(array, start1, end1, merge);
        int rightPairs = InversePairs(array, start2, end2, merge);
        int k = left;
        int pairs = 0;
        while (start1 <= end1 && start2 <= end2) {
            if (array[start1] > array[start2]) {
                pairs = pairs + end1 - start1 + 1;
                pairs = pairs % modBy;
        ...
        return (leftPairs + rightPairs + pairs) % modBy;
    }
    ...
    public int InversePairs(int[] array) {
        ...
        // Integer[] result = InversePairsCore2(array, 0, array.length - 1);//数值过大求余
        // System.out.println(Arrays.asList(result));
        // return count;

        int[] merge = new int[array.length];
        int res = InversePairsCore5(array, 0, array.length - 1, merge);
        res = res % 1000000007;
        return res;
    }

    private int InversePairsCore5(int[] array, int low, int high, int[] merge) {
        if (low >= high) {
            return 0;
        }
        ...
        int left = InversePairsCore5(array, start1, end1, merge);
        int right = InversePairsCore5(array, start2, end2, merge);
        int k = low;
        int count = 0;
        while (start1 <= end1 && start2 <= end2) {
            if (array[start1] > array[start2]) {
    ...
    private int InversePairsCore4(int[] array, int low, int high, int[] merge) {
        if (low == high) {
            // merge[low] = array[low];
            return 0;
        }
        int mid = (low + high) >> 1;
        int left = InversePairsCore4(array, low, mid, merge);
        int right = InversePairsCore4(array, mid + 1, high, merge);
        int i = low, j = mid + 1;
        int k = low;
        int count = 0;
        while (i <= mid && j <= high) {
    ...
    private Integer[] InversePairsCore3(int[] array, int low, int high) {
        if (low == high) {
            return new Integer[]{array[low]};
        }
        int mid = (low + high) >> 1;
        Integer[] left = InversePairsCore3(array, low, mid);
        Integer[] right = InversePairsCore3(array, mid + 1, high);
        int i = 0, j = 0;
        int leftLength = left.length;
        int rightLength = right.length;
        Integer[] merge = new Integer[leftLength + rightLength];
        int k = 0;
        while (i < leftLength && j < rightLength) {
            if (left[i] > right[j]) {
                while (j < rightLength && left[i] > right[j]) {
                    merge[k] = right[j];
                    k++;
                    j++;
                }
        ...
        if (j == rightLength) {
            count = count % 1000000007;
            count = count + (leftLength - i) * rightLength;
            for (; i < leftLength; i++) {
                merge[k] = left[i];
                k++;
            }
    ...
    private Integer[] InversePairsCore2(int[] array, int low, int high) {
        if (low == high) {
            return new Integer[]{array[low]};
        }
        int mid = (low + high) >> 1;
        Integer[] left = InversePairsCore2(array, low, mid);
        Integer[] right = InversePairsCore2(array, mid + 1, high);
        int i = 0, j = 0;
        int leftLength = left.length;
        int rightLength = right.length;
        Integer[] merge = new Integer[leftLength + rightLength];
